  ...
  return (builder: SchemaBuilder, schemaOptions: Options): void => {
    ...
    builder.hook("build", build => {
      ...
      if (Array.isArray(typeDefs)) {
        if(typeDefs.some(typeDef => (typeDef as any).kind !== "Document")) {
          throw new Error(
            "All elements given to an array passed as the first argument of makeExtendSchemaPlugin must be generated by the `gql` helper."
          );
        }
      } else {
        if (!(typeDefs as any) || (typeDefs as any).kind !== "Document") {
          throw new Error(
            "The first argument to makeExtendSchemaPlugin must be generated by the `gql` helper, or an array of the same."
          );
        }
      }

      const typeDefsArr = Array.isArray(typeDefs) ? typeDefs : [typeDefs];
      const mergedTypeDefinitions = typeDefsArr.reduce((definitions, typeDef) => definitions.concat(typeDef.definitions), [] as DefinitionNode[]);
