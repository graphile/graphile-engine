...
type ContextAndGenerators =
  | Context
  | {
      addDataGenerator: (a: DataGeneratorFunction) => undefined;
      addArgDataGenerator: (a: DataGeneratorFunction) => undefined;
...
const mergeData = (
  ...
  const results: Array<MetaData> | null | undefined = ensureArray(
  ...
  for (const result: MetaData of results) {
    for (const k of Object.keys(result)) {
      ...
      const newData: Array<unknown> | null | undefined = ensureArray(value);
...
function ensureArray<T>(val: undefined | Array<T> | T): undefined | Array<T> {
  if (val == null) {
    return;
...
export default function makeNewBuild(builder: SchemaBuilder): Build {
  ...
  return {
    ...
    addType(
      ...
      const newTypeSource =
        origin || // 'this' is typically only available after the build is finalized
        (this
