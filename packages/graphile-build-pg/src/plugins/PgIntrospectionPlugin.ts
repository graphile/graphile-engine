  ...
  introspectionResults.class.forEach(klass => {
    ...
    if (klass.tags.foreignKey) {
      ...
      foreignKeys.forEach((fkSpecRaw, index) => {
        ...
        const columns: string[] = parseSqlColumn(rawColumns, true); // $FlowFixMe

        const foreignSchema: string = parseSqlColumn(rawSchema); // $FlowFixMe

        const foreignTable: string = parseSqlColumn(rawTable); // $FlowFixMe
        ...
        const foreignKeyAttributeNums = attributesByNames(
          ...
          foreignColumns,
          `@foreignKey ${fkSpecRaw}`
        ).map(a => a.num); // Now we need to fake a constraint for this:
...
export default async function PgIntrospectionPlugin(
  ...
  builder.registerWatcher(async triggerRebuild => {
    ...
    await stopListening(); // Check we can get a pgClient
