...
import { PgClass } from "./plugins/PgIntrospectionPlugin"; // eslint-disable-next-line flowtype/no-weak-types
...
class QueryBuilder {
  ...
  makeLiveCollection(
    table: PgClass, // eslint-disable-next-line flowtype/no-weak-types
    ...
    if (this.parentQueryBuilder) {
      ...
      this.parentQueryBuilder.beforeLock("select", () => {
        const id = this.context.liveConditions.push(checkerGenerator) - 1; // BEWARE: it's easy to override others' conditions, and that will cause issues. Be sensible.
