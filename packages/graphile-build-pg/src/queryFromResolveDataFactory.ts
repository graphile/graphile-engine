...
import assert from "assert"; // eslint-disable-next-line flowtype/no-weak-types
...
export default (queryBuilderOptions: QueryBuilderOptions = {}) => (
  ...
  withBuilder?: ((builder: QueryBuilder) => undefined) | null | undefined,
  ...
  const rawCursorPrefix =
    reallyRawCursorPrefix && reallyRawCursorPrefix.filter(identity); // $FlowFixMe
  ...
  function generateNextPrevPageSql(
    ...
    if (!isForwardOrSymmetric) {
      assert(invert);
      assert(offset > 0); // We're looking for a previous page, and there's an offset, so lets just
      ...
      if (!queryHasBefore && !queryHasFirst) {
        assert(isForwardOrSymmetric); // There can be no next page since there's no upper bound
        ...
        assert(queryHasFirst); // queryHasBefore could be true or false.
      ...
      assert(!canHaveCursorInWhere); // We're dealing with LIMIT/OFFSET pagination here, which means `natural`
      ...
      if (limit == null) {
        ...
        assert(offset === 0); // Paginating backwards and there's no offset (which factors in before/after), so there's no previous page.
